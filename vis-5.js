 (function startWhenAIMReady() {
  ...
  `,
    ...
    const Vis5 = (() => {
      ...
      function makeGridStartPosition(builtPosition) {
        ...
        const distance = randomRange(TRANSITION.GRID_START_DISTANCE_MIN, TRANSITION.GRID_START_DISTANCE_MAX);

        const startPosition = builtPosition.clone().addScaledVector(direction, distance);

        startPosition.x += (Math.random() - 0.5) * TRANSITION.GRID_START_TANGENT_VARIANCE;
        startPosition.y += (Math.random() - 0.5) * TRANSITION.GRID_START_TANGENT_VARIANCE;
        startPosition.z += (Math.random() - 0.5) * TRANSITION.GRID_START_TANGENT_VARIANCE;

        return startPosition;
      }

      function makeStartQuaternion(builtQuaternion) {
        const randomRotation = new THREE.Euler((Math.random() - 0.5) * TRANSITION.GRID_START_ROT_VARIANCE, (Math.random() - 0.5) * TRANSITION.GRID_START_ROT_VARIANCE, (Math.random() - 0.5) * TRANSITION.GRID_START_ROT_VARIANCE);
      ...
      function makeGridTiming(row, col) {
        ...
        const delay = Math.min(TRANSITION.TI_GRID_DELAY_MAX, centreBias * TRANSITION.TI_GRID_DELAY_MAX + Math.random() * 0.08);

        const maxDuration = Math.max(0.0001, 1 - delay);

        const duration = Math.min(randomRange(TRANSITION.TI_GRID_DURATION_MIN, TRANSITION.TI_GRID_DURATION_MAX), maxDuration);
      ...
      function makeTileFieldPosition(sourcePosition) {
        ...
        const distance = randomRange(TRANSITION.TILE_OUT_DISTANCE_MIN, TRANSITION.TILE_OUT_DISTANCE_MAX);
      ...
      function makeFieldQuaternion() {
        return new THREE.Quaternion().setFromEuler(new THREE.Euler((Math.random() - 0.5) * TRANSITION.TILE_FIELD_ROT_VARIANCE, (Math.random() - 0.5) * TRANSITION.TILE_FIELD_ROT_VARIANCE, (Math.random() - 0.5) * TRANSITION.TILE_FIELD_ROT_VARIANCE));
      }
      ...
      function cloneTexturePatch(texture, patchSize, patchX, patchY) {
        ...
        const patchOffsetY = crop.offsetY + crop.scaleY - (patchY + 1) * patchScaleY;
      ...
      function makeFaceMaterials(patch) {
        ...
        return [makeSideMaterial(), makeSideMaterial(), makeSideMaterial(), makeSideMaterial(), makeImageMaterial(texture, patch.size, patch.x, patch.y), makeSideMaterial()];
      }
      ...
      function applyTileShading(tile, baseOpacity) {
        ...
        materials.forEach((mat, index) => {
          mat.opacity = index === 4 ? baseOpacity : baseOpacity * CONFIG.CUBE_SIDE_OPACITY;
      ...
      function imageWouldTouchDifferentPatch(patchMap, row, col, rows, cols, imageIndex, patchId) {
      ...
      function canPlacePatch(patchMap, row, col, rows, cols, size, imageIndex, patchId) {
        ...
        for (let y = 0; y < size; y++) {
          for (let x = 0; x < size; x++) {
            ...
            if (imageWouldTouchDifferentPatch(patchMap, targetRow, targetCol, rows, cols, imageIndex, patchId)) {
              return false;
            }
          }
        }
      ...
      function chooseImageForPatch(patchMap, row, col, rows, cols, size, patchId) {
        ...
        for (let attempt = 0; attempt < 20; attempt++) {
          ...
          if (canPlacePatch(patchMap, row, col, rows, cols, size, imageIndex, patchId)) {
            return imageIndex;
          }
        }
      ...
      function createImagePatchMap(rows, cols) {
        ...
        for (let row = 0; row < rows; row++) {
          for (let col = 0; col < cols; col++) {
            ...
            while (finalSize > 1) {
              imageIndex = chooseImageForPatch(patchMap, row, col, rows, cols, finalSize, patchId);
            ...
            if (imageIndex === null) {
              finalSize = 1;
              imageIndex = chooseImageForSingle(patchMap, row, col, rows, cols, patchId);
            }
      ...
      function getSvgInfluence(tile) {
        const passT = smoothstep(pct(CONFIG.SVG_PASS_START), pct(CONFIG.SVG_PASS_END), msProgress);

        const pad = CONFIG.SVG_PADDING_CELLS;

        if (tile.userData.row < pad || tile.userData.row >= CONFIG.GRID_ROWS - pad || tile.userData.col < pad || tile.userData.col >= CONFIG.GRID_COLUMNS - pad) {
          return 0;
        }
        ...
        if (localX < paddedLeft || localX > paddedRight || localY < paddedBottom || localY > paddedTop) {
          return 0;
        }

        if (Math.abs(localY - paddedCenterY) > svgHeightNormalized * 0.5 || Math.abs(localX - svgCenterX) > svgWidthNormalized * 0.5) {
          return 0;
        }
      ...
      function buildTiles() {
        ...
        for (let row = 0; row < rows; row++) {
          for (let col = 0; col < cols; col++) {
            ...
            tile.userData.toStagger = Math.random() * TRANSITION.TILE_TO_STAGGER_MAX;
  ...
  `,
      ...
      function applyTransitionIn() {
        const tileFadeT = easeOutCubic(mapRange(tiProgress, pct(TRANSITION.TI_CUBE_FADE_START), pct(TRANSITION.TI_CUBE_FADE_END), 0, 1));

        const floatT = easeOutCubic(mapRange(tiProgress, 0, pct(TRANSITION.TI_FLOAT_END), 0, 1));

        const gridTimeline = mapRange(tiProgress, pct(TRANSITION.TI_GRID_START), pct(TRANSITION.TI_GRID_END), 0, 1);

        tiles.forEach((tile) => {
          const delayed = getDelayedProgress(gridTimeline, tile.userData.pullDelay, tile.userData.pullDuration);

          const t = easeInOutCubic(Math.pow(delayed, TRANSITION.TI_EASE_POWER));

          tile.position.lerpVectors(tile.userData.startPosition, tile.userData.builtPosition, t);

          tile.quaternion.slerpQuaternions(tile.userData.startQuaternion, tile.userData.builtQuaternion, t);

          const scale = tile.userData.startScale + (tile.userData.builtScale - tile.userData.startScale) * t;
      ...
      function applyMainScroll() {
        tiles.forEach((tile) => {
          ...
          tile.userData.currentInfluence += (targetInfluence - tile.userData.currentInfluence) * CONFIG.SVG_RESPONSE_LERP;

          const easedInfluence = easeInOutCubic(tile.userData.currentInfluence);

          const scale = lerp(CONFIG.TILE_IDLE_SCALE, CONFIG.TILE_ACTIVE_SCALE, easedInfluence);
      ...
      function applyTransitionOut() {
        const breakupProgress = mapRange(toProgress, pct(TRANSITION.TO_TILE_BREAK_START), 1, 0, 1);

        const tileRawMove = mapRange(breakupProgress, 0, pct(TRANSITION.TO_TILE_MOVE_END), 0, 1);
        ...
        tiles.forEach((tile) => {
          const delayed = clamp01((tileRawMove - tile.userData.toStagger) / Math.max(1 - tile.userData.toStagger, 0.0001));

          const moveT = easeInOutCubic(Math.pow(delayed, TRANSITION.TI_EASE_POWER));

          tile.position.lerpVectors(tile.userData.builtPosition, tile.userData.fieldPosition, moveT);

          tile.quaternion.slerpQuaternions(tile.userData.builtQuaternion, tile.userData.fieldQuaternion, moveT);

          const scale = CONFIG.TILE_IDLE_SCALE + (tile.userData.fieldScale - CONFIG.TILE_IDLE_SCALE) * moveT;
      ...
      function updateRotationAndTransform() {
        ...
        pivotGroup.position.x = CONFIG.CUBE_POSITION_X + toProgress * TRANSITION.TO_MOVE_X;

        pivotGroup.position.y = CONFIG.CUBE_POSITION_Y + easeInOutCubic(toProgress) * TRANSITION.TO_MOVE_Y;

        pivotGroup.position.z = CONFIG.CUBE_POSITION_Z + toProgress * TRANSITION.TO_MOVE_Z;

        rubikRotationGroup.rotation.set(CONFIG.RUBIK_ROT_X + CONFIG.TILE_ANGLE_X, CONFIG.RUBIK_ROT_Y + CONFIG.TILE_ANGLE_Y, CONFIG.RUBIK_ROT_Z + CONFIG.TILE_ANGLE_Z);

        rubikRotationGroup.position.set(CONFIG.TILE_POSITION_X, CONFIG.TILE_POSITION_Y, CONFIG.TILE_POSITION_Z);
