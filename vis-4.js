console.log("Vis 4 - WED 5th AUG v6");
...
const TRANSITION = {
  ...
  // === TI FLOAT FIELD ===
  TI_FLOAT_END: 80, // Floating cubes finish building.
  TI_FLOAT_FROM_DISTANCE: 8.0, // Start below final position.
  TI_FLOAT_Y_VARIANCE: 0.5, // Small sideways variation while rising.

  // === TI GLOBE BUILD ===
  TI_GLOBE_START: 0, // Globe build starts.
  TI_GLOBE_END: 100, // Globe fully assembled.

  // Vertical rise distance.
  TI_GLOBE_RISE_DISTANCE_MIN: 2.5,
  TI_GLOBE_RISE_DISTANCE_MAX: 4.5,

  // Small sideways drift.
  TI_GLOBE_RISE_X_VARIANCE: 0.8,
  TI_GLOBE_RISE_Z_VARIANCE: 0.8,

  // Bottom of globe starts first.
  TI_GLOBE_VERTICAL_STAGGER: 0.35, // Less delay from bottom to top.
...
const Vis4 = (() => {
  ...
  function makeGlobeStartPosition(settledPosition) {
    const startPosition = settledPosition.clone();

    const riseDistance = randomRange(
      TRANSITION.TI_GLOBE_RISE_DISTANCE_MIN,
      TRANSITION.TI_GLOBE_RISE_DISTANCE_MAX
    );

    startPosition.y -= riseDistance;

    startPosition.x +=
      (Math.random() - 0.5) * TRANSITION.TI_GLOBE_RISE_X_VARIANCE;

    startPosition.z +=
      (Math.random() - 0.5) * TRANSITION.TI_GLOBE_RISE_Z_VARIANCE;
  ...
  function makeOffsetBuiltPosition(builtPosition, candidate) {
    ...
    const levelT = Math.max(
      candidate.offsetT || 0,
      candidate.minOffsetLevelBySize || 0
    );

    const jitter = randomRange(
      -CONFIG.GLOBE_TILE_OFFSET_JITTER,
      CONFIG.GLOBE_TILE_OFFSET_JITTER
    );

    const offsetAmount =
      lerp(CONFIG.GLOBE_TILE_OFFSET_MIN, CONFIG.GLOBE_TILE_OFFSET_MAX, levelT) +
      jitter;

    const clampedOffset = Math.min(
      Math.max(offsetAmount, CONFIG.GLOBE_TILE_OFFSET_MIN),
      CONFIG.GLOBE_TILE_OFFSET_MAX
    );
  ...
  function makeGlobeTiming(settledPosition) {
    ...
    const verticalT = clamp01(
      (settledPosition.y + verticalRange) / Math.max(verticalRange * 2, 0.0001)
    );

    const delay =
      verticalT * TRANSITION.TI_GLOBE_VERTICAL_STAGGER +
      Math.random() * TRANSITION.TI_GLOBE_RANDOM_STAGGER;

    return {
      delay,
      duration: Math.max(0.0001, 1 - delay)
    };
  }
  ...
  function makeFloatIntroPosition(finalPosition) {
    ...
    introPosition.y -= TRANSITION.TI_FLOAT_FROM_DISTANCE;

    introPosition.x += (Math.random() - 0.5) * TRANSITION.TI_FLOAT_Y_VARIANCE;

    introPosition.z += (Math.random() - 0.5) * TRANSITION.TI_FLOAT_Y_VARIANCE;
  ...
  function getCubeRearFade(cubelet) {
    ...
    tempCubeViewPosition
      .copy(tempCubeWorldPosition)
      .applyMatrix4(app.camera.matrixWorldInverse);

    tempGlobeViewPosition
      .copy(tempGlobeWorldPosition)
      .applyMatrix4(app.camera.matrixWorldInverse);

    const rearDepth = tempGlobeViewPosition.z - tempCubeViewPosition.z;

    return smoothstep(
      CONFIG.GLOBE_REAR_FADE_START,
      CONFIG.GLOBE_REAR_FADE_END,
      rearDepth
    );
  }
  ...
  function applyGlobeShading(cubelet, baseOpacity) {
    ...
    const opacity =
      baseOpacity *
      THREE.MathUtils.lerp(1, CONFIG.GLOBE_REAR_OPACITY, rearFade);

    const brightness = THREE.MathUtils.lerp(
      1,
      CONFIG.GLOBE_REAR_BRIGHTNESS,
      rearFade
    );
  ...
  function cloneTexturePatch(
    ...
    t.offset.set(
      crop.offsetX + (patchX + 1) * patchScaleX,
      crop.offsetY + crop.scaleY - patchY * patchScaleY
    );
  ...
  function makeFaceMaterials(patch, candidate) {
    ...
    return [
      makeImageMaterial(texture, patch.size, patch.x, patch.y, targetAspect),
      makeSideMaterial()
    ];
  }
  ...
  function getNextImageIndex() {
    if (
      shuffledImageIndexes.length === 0 ||
      imagePickIndex >= shuffledImageIndexes.length
    ) {
    ...
    while (
      recentImageIndexes.includes(imageIndex) &&
      attempts < shuffledImageIndexes.length
    ) {
  ...
  function createTileFootprints(rows, cols) {
    ...
    function canPlacePlannedFootprint(row, col, size, sameSizeGap) {
      ...
      if ((touchesTopCap || touchesBottomCap) && capTileCount >= 1)
        return false;
    ...
    function assignBalancedOffsets() {
      ...
      for (let i = 0; i < footprints.length; i++) {
        ...
        const offsetT =
          levelIndex / Math.max(CONFIG.GLOBE_TILE_OFFSET_LEVELS - 1, 1);
    ...
    placeDistributedSize(
      3,
      CONFIG.GLOBE_TILE_3X3_COUNT,
      CONFIG.GLOBE_TILE_3X3_ROW_PADDING,
      1 - CONFIG.GLOBE_TILE_3X3_ROW_PADDING,
      0.35,
      1
    );
    ...
    placeDistributedSize(
      2,
      CONFIG.GLOBE_TILE_2X2_COUNT,
      CONFIG.GLOBE_TILE_2X2_ROW_PADDING,
      1 - CONFIG.GLOBE_TILE_2X2_ROW_PADDING,
      0.45,
      1
    );
    ...
    const smallCount = Math.floor(
      remainingCells * CONFIG.GLOBE_TILE_1X1_DENSITY
    );

    placeDistributedSize(
      1,
      smallCount,
      CONFIG.GLOBE_TILE_1X1_ROW_PADDING,
      1 - CONFIG.GLOBE_TILE_1X1_ROW_PADDING,
      0.65,
      1
    );
  ...
  function makeSphericalTileGeometry(candidate) {
    ...
    const latTop = lerp(
      candidate.latCenter,
      candidate.latTop,
      1 - CONFIG.TILE_GAP_LAT
    );
    const latBottom = lerp(
      candidate.latCenter,
      candidate.latBottom,
      1 - CONFIG.TILE_GAP_LAT
    );
    ...
    for (let x = 0; x < CONFIG.SURFACE_SEGMENTS_X; x++) {
      ...
      const bottomFrontA =
        CONFIG.SURFACE_SEGMENTS_Y * (CONFIG.SURFACE_SEGMENTS_X + 1) + x;
  ...
  function buildCubelets() {
    ...
    candidates.forEach((candidate) => {
      ...
      const cubelet = new THREE.Mesh(
        built.geometry,
        makeFaceMaterials(patch, candidate)
      );
      ...
      const startPosition = makeGlobeStartPosition(settledPosition);
      const startQuaternion = makeStartQuaternion(builtQuaternion);
      const fieldScale = makeFieldScale();
      const timing = makeGlobeTiming(settledPosition);
  ...
  function applyTransitionIn() {
    ...
    const globeTimeline = mapRange(
      tiProgress,
      pct(TRANSITION.TI_GLOBE_START),
      pct(TRANSITION.TI_GLOBE_END),
      0,
      1
    );
    ...
    cubelets.forEach((cubelet) => {
      ...
      cubelet.position.lerpVectors(
        ...
        cubelet.userData.settledPosition,
        t
      );

      cubelet.quaternion.slerpQuaternions(
        cubelet.userData.startQuaternion,
        cubelet.userData.builtQuaternion,
        t
      );

      const scale =
        cubelet.userData.startScale +
        (cubelet.userData.builtScale - cubelet.userData.startScale) * t;
  ...
  function applyTransitionOut() {
    ...
    cubelets.forEach((cubelet) => {
      ...
      const scale =
        cubelet.userData.builtScale +
        (cubelet.userData.fieldScale - cubelet.userData.builtScale) * moveT;
