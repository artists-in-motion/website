...
const Vis6 = (() => {
  ...
  function getTiProgress() {
    const rawT = inverseLerp(
      CONFIG.TI_OPACITY_START / 100,
      CONFIG.TI_OPACITY_END / 100,
      tiProgress
    );
  ...
  function getTiOpacity() {
    ...
    return (
      CONFIG.TI_OPACITY_FROM +
      (CONFIG.TI_OPACITY_TO - CONFIG.TI_OPACITY_FROM) * t
    );
  }
  ...
  function createTiledImagePlane(texture) {
    ...
    const alphaData = sampleAlphaMask(image, cols, rows);
    ...
    for (let y = 0; y < rows; y++) {
      ...
      for (let x = 0; x < cols; x++) {
        ...
        const gap = randomRange(
          CONFIG.BUILD_COLUMN_GAP_MIN,
          CONFIG.BUILD_COLUMN_GAP_MAX
        );

        cumulativeColumnOffsets[x] += gap;

        const buildYOffset =
          cumulativeColumnOffsets[x] * CONFIG.BUILD_COLUMN_GAP_DIRECTION;
    ...
    geometry.setAttribute(
      "instanceGridPos",
      new THREE.InstancedBufferAttribute(new Float32Array(gridPositions), 2)
    );

    geometry.setAttribute(
      "instanceRandom",
      new THREE.InstancedBufferAttribute(new Float32Array(randoms), 1)
    );

    geometry.setAttribute(
      "instanceRowProgress",
      new THREE.InstancedBufferAttribute(new Float32Array(rowProgresses), 1)
    );

    geometry.setAttribute(
      "instanceBuildYOffset",
      new THREE.InstancedBufferAttribute(new Float32Array(buildYOffsets), 1)
    );
  ...
  function createFloatLayer(settings) {
    ...
    const floatHeight =
      columnTotalHeight * settings.spreadYMultiplier + settings.extraHeight;
    ...
    geometry.setAttribute(
      "aPlaneDistance",
      new THREE.InstancedBufferAttribute(planeDistanceArray, 1)
    );
    ...
    for (let i = 0; i < count; i++) {
      ...
      const position = new THREE.Vector3(
        randomSigned() * floatWidth * 0.5,
        randomSigned() * floatHeight * 0.5,
        z
      );

      const planeDistance = Math.abs(z);

      const distanceT = clamp01(
        (planeDistance - CONFIG.FLOAT_NEAR_Z) /
          Math.max(CONFIG.FLOAT_FAR_Z - CONFIG.FLOAT_NEAR_Z, 0.0001)
      );

      const scale =
        settings.closeImageScale +
        (settings.farImageScale - settings.closeImageScale) * distanceT;
  ...
  function updateFloatLayer(layer, tiOpacity) {
    ...
    layer.material.uniforms.uOpacity.value =
      layer.opacity * tiOpacity * (1.0 - toProgress);
  ...
  function layoutColumn() {
    ...
    for (let i = 1; i < imagePlanes.length; i++) {
      ...
      currentY -=
        previous.height * 0.5 + current.height * 0.5 + CONFIG.GRID_GAP_Y;
  ...
  function updateTransitionProgress(progress = {}) {
    ...
    msProgress = clamp01((localY - tiPx) / msPx);

    toProgress = clamp01((localY - sectionHeight) / Math.max(toPx, 1));
  }

  function applyTimeline() {
    const vh = Math.max(lastViewportHeight, 1);

    const sectionHeight = sectionEl?.offsetHeight || 1;

    const tiPx = vh * (TRANSITION.TI / 100);

    const toPx = vh * (TRANSITION.TO / 100);

    const msPx = Math.max(sectionHeight - tiPx, 1);

    const travelDistance = Math.abs(lastImageBaseY - firstImageBaseY);

    // First image moves through the build zone during TI.
    const tiOffset = CONFIG.BUILD_ZONE_START_Y * (1 - tiProgress);

    // Whole image stack travels from first image to last across MS.
    const msOffset = travelDistance * msProgress;

    // Continue at the same MS speed through TO.
    const toBaseContinuation = toProgress * toPx * (travelDistance / msPx);

    // Preserve the original additional TO acceleration.
  ...
  function updateGridState() {
    ...
    for (let i = 0; i < imagePlanes.length; i++) {
      ...
      const imageBuildProgress = easeOutCubic(
        inverseLerp(
          CONFIG.BUILD_ZONE_START_Y,
          CONFIG.BUILD_ZONE_END_Y,
          imageSceneY
        )
      );
  ...
  return {
    ...
    resize(app, progress = {}) {
      ...
      const currentProgress = app?.scroll?.progressById?.["vis-6"] || progress;
