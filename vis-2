  ...
  `;
    ...
    const CONFIG = {
      ...
      // SVG IMAGE MAPPING
      SVG_IMAGE_URL: 'https://cdn.prod.website-files.com/69dec44200d5fa5789162235/6a0d470fce8ee3c056e98343_services-cc-6.jpg',
  ...
  `;
    ...
    const Vis2 = (() => {
      ...
      function sampleSvgMaskAtWorld(worldX, worldY) {
        ...
        const localX = (worldX - CONFIG.SVG_MASK_POSITION_X) / Math.max(getSvgMaskWidth(), 0.0001);

        const localY = (worldY - CONFIG.SVG_MASK_POSITION_Y) / Math.max(CONFIG.SVG_MASK_HEIGHT, 0.0001);
      ...
      function buildParticleData() {
        ...
        for (let y = 0; y < rows; y++) {
          for (let x = 0; x < cols; x++) {
            ...
            const liftHeight = randomRange(CONFIG.SVG_LIFT_MIN_HEIGHT, CONFIG.SVG_LIFT_MAX_HEIGHT);
            ...
            if (mask > 0.5) {
              ...
              for (let layer = 0; layer < layerCount; layer++) {
                const layerOffset = layer * CONFIG.SVG_EXTRUDE_LAYER_HEIGHT + randomRange(-CONFIG.SVG_EXTRUDE_HEIGHT_VARIATION, CONFIG.SVG_EXTRUDE_HEIGHT_VARIATION);
      ...
      function buildParticles() {
        ...
        riverGeometry.setAttribute('position', new THREE.BufferAttribute(data.positions, 3));
        riverGeometry.setAttribute('aStartPosition', new THREE.BufferAttribute(data.startPositions, 3));
        riverGeometry.setAttribute('aRowIndex', new THREE.BufferAttribute(data.rowIndices, 1));
        riverGeometry.setAttribute('aToDelayMs', new THREE.BufferAttribute(data.toDelays, 1));
        riverGeometry.setAttribute('aToDistance', new THREE.BufferAttribute(data.toDistances, 1));
        riverGeometry.setAttribute('aToMoveStrength', new THREE.BufferAttribute(data.toMoveStrengths, 1));
        riverGeometry.setAttribute('aSvgMask', new THREE.BufferAttribute(data.svgMasks, 1));
        riverGeometry.setAttribute('aSvgLiftDelay', new THREE.BufferAttribute(data.svgLiftDelays, 1));
        ...
        liftGeometry.setAttribute('position', new THREE.BufferAttribute(data.liftPositions, 3));
        liftGeometry.setAttribute('aStartPosition', new THREE.BufferAttribute(data.liftPositions, 3));
        liftGeometry.setAttribute('aRowIndex', new THREE.BufferAttribute(data.liftRowIndices, 1));
        liftGeometry.setAttribute('aSvgLiftDelay', new THREE.BufferAttribute(data.liftDelays, 1));
        liftGeometry.setAttribute('aSvgLiftHeight', new THREE.BufferAttribute(data.liftHeights, 1));
        liftGeometry.setAttribute('aSvgLayerOffset', new THREE.BufferAttribute(data.liftLayerOffsets, 1));
        liftGeometry.setAttribute('aSvgU', new THREE.BufferAttribute(data.liftUs, 1));
        liftGeometry.setAttribute('aSvgV', new THREE.BufferAttribute(data.liftVs, 1));
        liftGeometry.setAttribute('aSvgLayerIndex', new THREE.BufferAttribute(data.liftLayerIndices, 1));
      ...
      function applyTransitionIn() {
        const fadeT = Math.pow(easeOutCubic(tiProgress), TRANSITION.TI_FADE_POWER);
      ...
      function updateWaveOffset() {
        ...
        const toFadeT = getRangeProgress(toProgress, 0.0, CONFIG.AUTO_WAVE_TO_FADE_END);
      ...
      function updateRiverScrollMove() {
        ...
        riverScrollMoveX = baseMoveX + toProgress * TRANSITION.TO_SCROLL_CONTINUE_X;
      ...
      function updateMaterialUniforms() {
        if (riverMaterial && riverPoints) {
          ...
          riverMaterial.uniforms.uPointSize.value = CONFIG.POINT_SIZE * renderer.getPixelRatio();
          riverMaterial.uniforms.uWaveAmplitude.value = CONFIG.WAVE_AMPLITUDE + waveBoost;
          ...
          riverMaterial.uniforms.uBackgroundAlphaMultiplier.value = CONFIG.BACKGROUND_ALPHA_MULTIPLIER;
          riverMaterial.uniforms.uSvgAlphaMultiplier.value = CONFIG.SVG_ALPHA_MULTIPLIER;
          riverMaterial.uniforms.uBackgroundSizeMultiplier.value = CONFIG.BACKGROUND_SIZE_MULTIPLIER;
          riverMaterial.uniforms.uSvgSizeMultiplier.value = CONFIG.SVG_SIZE_MULTIPLIER;
          ...
          riverMaterial.uniforms.uToOpacityBoost.value = TRANSITION.TO_OPACITY_BOOST;
          ...
          riverPoints.rotation.x = CONFIG.BASE_ROT_X + toProgress * TRANSITION.TO_OBJECT_ROTATE_X;
        }
        ...
        if (liftMaterial && liftPoints) {
          ...
          liftMaterial.uniforms.uPointSize.value = CONFIG.POINT_SIZE * renderer.getPixelRatio();
          liftMaterial.uniforms.uWaveAmplitude.value = CONFIG.WAVE_AMPLITUDE + waveBoost;
          ...
          liftMaterial.uniforms.uSvgLiftFadePower.value = CONFIG.SVG_LIFT_FADE_POWER;
          liftMaterial.uniforms.uSvgColor.value.set(CONFIG.SVG_COLOR);
          liftMaterial.uniforms.uUseSvgTexture.value = CONFIG.SVG_IMAGE_URL ? 1.0 : 0.0;
          liftMaterial.uniforms.uSvgImageMode.value = CONFIG.SVG_IMAGE_MODE === 'layerTiles' ? 1.0 : 0.0;
          liftMaterial.uniforms.uSvgLayerCount.value = Math.max(1, Math.floor(CONFIG.SVG_EXTRUDE_LAYERS));
          liftMaterial.uniforms.uSvgLayerCropShift.value = CONFIG.SVG_IMAGE_LAYER_CROP_SHIFT;
          liftMaterial.uniforms.uSvgImageMix.value = CONFIG.SVG_IMAGE_MIX;
          liftMaterial.uniforms.uSvgImageBrightness.value = CONFIG.SVG_IMAGE_BRIGHTNESS;
